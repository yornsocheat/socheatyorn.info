...
const renderExperiences = (experiences, elementID) => {
    ...
    if (experiences) {
        ...
        h3.textContent = 'Employment History';
        h3.className = 'mb-3';
        h3.appendChild(i);
        experiencesDiv.appendChild(h3);

        experiences.forEach(experience => {
            if (experience.jobTitle && experience.employer && experience.startDate && experience.endDate && experience.city) {
...
const renderEducations = (educations, elementID) => {
    ...
    if (educations) {
        ...
        educations.forEach(education => {
            if (education.school && education.degree && education.startDate && education.endDate && education.city) {
                let h5 = document.createElement('h5');
                let date = document.createElement('p');
...
const renderReferences = (references, elementID) => {
    ...
    if (references) {
        ...
        references.forEach(reference => {
            if (reference.fullName && reference.company && reference.phone) {
                let h5 = document.createElement('h5');
                let p = document.createElement('p');
                h5.textContent = `${reference.fullName} from ${reference.company}`;
                h5.className = 'mb-1 tsize-18';
                p.innerHTML = `+${reference.phone} ${reference.email != '' ? ', ' + reference.email : ''}`;
                p.className = 'tsize-14 text-gray mb-4';
...
const renderDetails = (details, elementID) => {
    ...
    if (details && details.address && details.phone && details.email) {
        ...
        p.innerHTML = `${details.address}<br/>${details.email}<br/>Phone: +${details.phone}${(details.skype && details.skype !='') ? '<br/>Skype Name: ' + details.skype : ''}`;
        detailsDiv.appendChild(p);
    }
};

const renderHobits = (hobits, elementID) => {
    let hobitsDiv = document.querySelector(elementID);
    hobits.forEach(hobit => {
        let p = document.createElement('p');
        p.className = 'tsize-14';
        p.textContent = `${hobit}`;
        hobitsDiv.appendChild(p);
    });
};
    ...
    if (links) {
        links.forEach(link => {
            ...
            if (link.link.length > 1) {
                ...
                p.innerHTML = `<a href="${link.link}" class="text-blue">${link.title}</a>`;
            }
